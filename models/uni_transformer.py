...
# 数据准备
# 假设我们有两个分子，每个分子有4个原子
# 原子的特征向量（hidden_dim=16）
h1 = torch.randn(4, 16)  # 分子1的原子特征
h2 = torch.randn(4, 16)  # 分子2的原子特征
h = torch.cat([h1, h2], dim=0)  # 合并特征

# 原子的空间坐标（3D）
x1 = torch.randn(4, 3)  # 分子1的原子坐标
x2 = torch.randn(4, 3)  # 分子2的原子坐标
x = torch.cat([x1, x2], dim=0)  # 合并坐标

# 原子类型（假设每个分子有4种类型）
mask_ligand1 = torch.tensor([1, 1, 0, 0])  # 分子1的原子类型（1表示配体，0表示其他）
mask_ligand2 = torch.tensor([1, 0, 1, 0])  # 分子2的原子类型
mask_ligand = torch.cat([mask_ligand1, mask_ligand2], dim=0)  # 合并类型

# 分子的批次信息（batch）
batch = torch.tensor([0, 0, 0, 0, 1, 1, 1, 1])  # 分子1的原子属于batch 0，分子2的原子属于batch 1

# 创建图数据
data = Data(x=x, h=h, mask_ligand=mask_ligand, batch=batch)

# 模型实例化
model = UniTransformerO2TwoUpdateGeneral(
    num_blocks=1,  # 块的数量
    num_layers=1,  # 每个块的层数
    hidden_dim=16,  # 隐藏层维度
    n_heads=4,  # 多头注意力的头数
    k=32,  # KNN图的K值
    num_r_gaussian=20,  # 高斯扩展的维度
    edge_feat_dim=4,  # 边特征的维度
    num_node_types=20,  # 节点类型的数量
    act_fn='relu',  # 激活函数
    norm=True,  # 是否使用归一化
    cutoff_mode='radius',  # 截断模式
    ew_net_type='r',  # 边权重网络类型
    num_init_x2h=1,  # 初始层的X2H层数量
    num_init_h2x=0,  # 初始层的H2X层数量
    num_x2h=1,  # 每层的X2H层数量
    num_h2x=1,  # 每层的H2X层数量
    r_max=10.0,  # 最大距离
    x2h_out_fc=True,  # 是否使用X2H输出的全连接层
    sync_twoup=False  # 是否同步更新
)

# 模型调用
outputs = model(h=data.h, x=data.x, mask_ligand=data.mask_ligand, batch=data.batch, return_all=True, fix_x=False)

# 打印输出
print("最终原子坐标 x:", outputs['x'])
print("最终原子特征 h:", outputs['h'])
print("所有中间原子坐标 all_x:", outputs['all_x'])
print("所有中间原子特征 all_h:", outputs['all_h'])